...
--Inserção da coluna Data de Nascimento na Tabela Usuario
alter table usuario 
add column usu_dt_nasc date;
...
;
--Atualização com Inserção das Datas de Nascimento dos Usuários
update usuario set usu_dt_nasc = '11/01/2005' where usu_cd_id = '111';
update usuario set usu_dt_nasc = '28/08/1998' where usu_cd_id = '222';
update usuario set usu_dt_nasc = '23/06/1997' where usu_cd_id = '333';
update usuario set usu_dt_nasc = '16/09/1992' where usu_cd_id = '444';
update usuario set usu_dt_nasc = '11/12/2001' where usu_cd_id = '555';
...
--Exclusão de Produto caso esteja em Indisponível
delete from produto 
where prod_cd_id = 11;

select * from produto 

--Filtro para verificar produtos com valor abaixo do escolhido
SELECT
  count(*)
FROM
  produto
...
values ('1', current_timestamp, '4', '111');
...
where prod_cd_id = 12 and item_cd_id = 4 
GROUP BY item_int_qtd, prod_nm_valor_unitario;

--Criação de uma View para Nota Fiscal
...
--Criação de uma view para Implementação de uma view para as categoris serem mais fluidas
 create view informatica as
 select prod_tx_nome, prod_tx_descricao
 from produto
 where prod_cd_id = 12 or prod_cd_id = 13 or prod_cd_id = 14 or prod_cd_id = 15
 
 select * from informatica
 
--Filtro para a visualização de produtos e categorias da escolha do usuários
 SELECT 
	prod_tx_nome, cat_tx_nome
FROM 
	produto
INNER JOIN 
	categoria
ON 
	produto.fk_cat_cd_id  = categoria.cat_cd_id;
